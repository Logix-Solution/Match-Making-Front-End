...
export class PreferencesCareerComponent implements OnInit {
  ...
  getOccupationTitle(subTypeID: number | undefined): string {
    if (!subTypeID) return '';
    return this.occupationList.find(o => o.subTypeID === subTypeID)?.subTypeTitle || '';
  }
  ...
  syncFormFields(): void {
    ...
    if (this.selectedMinEducation) {
      careerArray.push({ typeID: 4, subTypeID: Number(this.selectedMinEducation), priority: 1 });
    }

    // Min Monthly Income — typeID 6
    if (this.selectedMinIncome) {
      careerArray.push({ typeID: 6, subTypeID: Number(this.selectedMinIncome), priority: 1 });
    }

    // Preferred Occupations — typeID 5, priority = selection order (1,2,3)
    this.selectedOccupations.forEach((subTypeID, i) => {
      careerArray.push({ typeID: 5, subTypeID: Number(subTypeID), priority: i + 1 });
    });
  ...
  save(): void {
    ...
    // ─── Manual validations ───────────────────────────────────────────────
    if (!this.selectedMinEducation) {
      this.toastr.warning('Please select preferred minimum education level'); return;
    }
    if (!this.selectedMinIncome) {
      this.toastr.warning('Please select preferred minimum monthly income'); return;
    }
    if (this.selectedOccupations.length === 0) {
      this.toastr.warning('Please select at least one occupation preference'); return;
    }
