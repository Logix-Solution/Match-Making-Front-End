...
export class PreferencesLifestyleComponent implements OnInit {
  ...
  syncFormFields(): void {
    ...
    if (this.selectedSmoke)    lifestyleArray.push({ typeID: 17, subTypeID: Number(this.selectedSmoke)    });
    if (this.selectedAlcohol)  lifestyleArray.push({ typeID: 18, subTypeID: Number(this.selectedAlcohol)  });
    if (this.selectedWantKids) lifestyleArray.push({ typeID: 19, subTypeID: Number(this.selectedWantKids) });
  ...
  save(): void {
    ...
    if (!this.selectedSmoke) {
      this.toastr.warning('Please select smoking preference'); return;
    }
    if (!this.selectedAlcohol) {
      this.toastr.warning('Please select alcohol preference'); return;
    }
    if (!this.selectedWantKids) {
      this.toastr.warning('Please select partner want kids preference'); return;
    }
