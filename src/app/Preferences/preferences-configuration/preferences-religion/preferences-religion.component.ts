...
export class PreferencesReligionComponent implements OnInit {
  ...
  save(): void {
    // ─── Manual validations ───────────────────────────────────────────────
    if (!this.selectedReligion) {
      this.toastr.warning('Please select preferred religion');
      return;
    }
    if (!this.selectedSect) {
      this.toastr.warning('Please select preferred sect');
      return;
    }
    if (!this.selectedReligionImportance) {
      this.toastr.warning('Please select how important religion is to partner');
      return;
    }
  ...
  loadUserDetails(): void {
    ...
    this.dataService
      ...
      .subscribe({
        ...
        error: (err: any) =>
          console.log('Religion Preference Load Error:', err),
      });
  }
}
