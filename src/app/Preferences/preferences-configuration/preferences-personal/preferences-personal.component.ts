...
interface PersonalPreferenceInterface {
  userID: number; // 0
  spType: string; // 1
  nationalityID: number; // 2  → 0, unused (kept for backward compatibility)
  personalPrefrence: string; // 3  → [{typeID,subTypeID}, ...] (cast + ethnicity + ages only)
  cityID: number; // 4
  nationality: number; // 5  → dedicated country_id field, same pattern as ProfilePersonalInfoInputComponent
}
    ...
    this.dataService
      ...
      .subscribe({
        next: (response: any) => {
          ...
          // city/country/nationality — all live in userPreference with countryID key
