...
//     this.pageFields.countryCodeID = this.selectedCountryCodeID as number;
...
//   //           this.router.navigate(['/login']);
//   //         } 
//   //         else if (apiResponse?.includes('Email already Exist')) {
//   //             this.showOtpModal = false;
...
//         // Log the raw response so we can see exactly what the API sent back
...
//           this.valid.apiErrorResponse('Email already exists');
//         } else {
//           // Unknown/other error — close modal too, otherwise it just sits there
//           // with no feedback, which is exactly the "stuck modal" symptom.
//           this.showOtpModal = false;
