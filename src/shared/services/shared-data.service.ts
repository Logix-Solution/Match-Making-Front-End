...
Message: ${error.message}`;
  ...
  resetPassword(payload: {
    ...
    return this.http
      .post(`${this.baseURL}user-api/forgetPassword`, payload)
      .pipe(
        retry(0),
        catchError((error) => this.handleError(error)),
      );
...
//       // server returned a plain array body, e.g. ["CNIC already exists"]
//       errorMessage = error.error[0];
//     } else if (error?.error?.message) {
//       // server returned a structured error body with a message
//       errorMessage = error.error.message;
//     } else if (typeof error?.error === 'string' && error.error.trim() !== '') {
//       // server returned a plain string error body
//       errorMessage = error.error;
//     } else if (error?.message) {
//       // fallback to the HttpErrorResponse's own message
//       errorMessage = `Error Code: ${error.status}\nMessage: ${error.message}`;
//     }
